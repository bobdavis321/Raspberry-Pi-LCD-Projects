...
# RPI ILI9340 driver
  ...
  Write_CD((0xB6, 0x0a, 0x82, 0x27, 0x00))# Display Function Control 
  ...
  for row in range(20): # 20 rows top to bottom
    ...
    for char in string: # Procees each character in string
      for byte in range(5):
        ...
        pix=0
        if (row==0) and (data[byte] & 0x01):pix=1
        if (row==1) and (data[byte] & 0x01):pix=1
        if (row==2) and (data[byte] & 0x02):pix=1
        if (row==3) and (data[byte] & 0x02):pix=1
        if (row==4) and (data[byte] & 0x04):pix=1
        if (row==5) and (data[byte] & 0x04):pix=1
        if (row==6) and (data[byte] & 0x08):pix=1
        if (row==7) and (data[byte] & 0x08):pix=1
        if (row==8) and (data[byte] & 0x10):pix=1
        if (row==9) and (data[byte] & 0x10):pix=1
        if (row==10) and (data[byte] & 0x20):pix=1
        if (row==11) and (data[byte] & 0x20):pix=1
        if (row==12) and (data[byte] & 0x40):pix=1
        if (row==13) and (data[byte] & 0x40):pix=1
        if (row==14) and (data[byte] & 0x80):pix=1
        if (row==15) and (data[byte] & 0x80):pix=1
